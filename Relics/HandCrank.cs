...
public class HandCrank : CustomRelicModel
{
    public override RelicRarity Rarity => RelicRarity.Starter;

    // 标记是否有待消耗的应力（用于跨越同步/异步边界）
    private bool _pendingStressConsume = false;

    // ① 每场战斗开始时获得 1 层应力
    public override async Task BeforeCombatStart()
    {
        Flash();
        await CommonActions.Apply<StressPower>(Owner.Creature, null, 1m);
    }
        ...
        CardModel? cardSource)
    {
        ...
        if (stress == null || stress.Amount <= 0) return amount;
